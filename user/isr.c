...
IFX_INTERRUPT(cc61_pit_ch0_isr, CCU6_1_CH0_INT_VECTAB_NUM, CCU6_1_CH0_ISR_PRIORITY)
{
    ...
    GetSpeed();
    float gFacing = pid(&Vz, 0, (Encoder_speed_l+Encoder_speed_r)/2);
    mpu6050_get_acc();
    float speed = pid(&pitch,0.08, -imu660ra_acc_transition(mpu6050_acc_y));
    speed = 10;
    MotorSetPWM(
            pid(&motorL, speed, Encoder_speed_l),
            pid(&motorR, speed, Encoder_speed_r)
            );
}
